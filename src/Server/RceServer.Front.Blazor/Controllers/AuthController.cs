...
namespace RceServer.Front.Blazor.Controllers
{
	...
	public class AuthController : Controller
	{
		...
		//	{
		//		return Ok(new ErrorDto { Error = "Passwords don't match" });
		//	}
		...
		[HttpPost("refresh")]
		public async Task<IActionResult> Refresh([FromBody] RefreshTokenResponseModel model)
		{
			var authority = $"{_configuration["Authority"]}/connect/token";
			var clientSecret = _configuration["ClientSecret"];
			var httpResponseMessage = await _httpClientService.Post(authority,
		...
		public async Task<IActionResult> CheckEmail(string userId, string code)
		{
			...
			{
				return Ok(JsonConvert.SerializeObject(new ErrorDto
				{
		...
		public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordModel model)
		{
			...
			return Ok(new ErrorDto { Error = "Password reset error" });
		}
	}
}
